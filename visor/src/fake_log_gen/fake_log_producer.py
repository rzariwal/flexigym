...
class fake_access_producer(fake_log_gen.fake_access_gen):
    ...
    def access_lines(self):
        while True:
            ...
            jsondata = {'IP': ip, 'user_identifier': user_identifier, 'user_id': user_id, 'user_name': user_name,
                        'time': t, 'Message': msg, 'code': code,
                        'size': size, 'device': device, 'user_name': user_name, 'tenant_id': tenant_id,
                        'timezone': timezone, 'OS': OS, 'browser': browser,
                        'country': country, 'screenResolution': screenResolution, 'action': action,
                        'referrer': referrer, 'timezone': timeonpage,
                        'supplier_id': supplier_id, 'product': product, 'geolocation': geolocation}

            data = '%s %s %s %s [%s] "%s" %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s' % (
                ip, user_identifier, user_id, user_name, t, msg, code, size, device,
            ...
            # add to login topic if it is a login event
            if action is "login":
                self.producer.send(self.login_topic, (data + '\n').encode())
